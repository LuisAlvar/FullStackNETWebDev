...
- Create Angular project with ng: `ng new imager.client --defaults --skip-install --skip-git --no-standalone `.
...
- Create project file (`imager.client.esproj`).
- Create `launch.json` to enable debugging.
- Create `nuget.config` to specify location of the JavaScript Project System SDK (which is used in the first line in `imager.client.esproj`).
...
The following steps were use to complete the bussiness requriments:
- Create new Augular component `ng generate component UplooadForm` --skip-tests --module=app
- Update `app.module.ts` adding UploadFormComponent under declarations and FormsModule;ReactiveFormsModule under imports
